...
sub BookmarkDelete {
    ...
    my $raObject;
    $raObject[9]->oDocument($name)->Delete();
...
sub GetFileTemp {
    ...
    my @aPackage =
            grep { $_->location->[0] <= $row }
                    $self->MarkDocumentAsSaved("PPI::Statement::Package")
                            or return("main");
